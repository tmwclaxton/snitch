...
`CompetitorInsightsBuilder::forUser` powers Dashboard mix, hashtags, keywords, CTA language, growth, follower history, paid vs organic, posts with an ask, a short Ad Library preview, and a short playbook. Per-snitch Show still uses `forAccount`. Growth comes from daily `follower_snapshots` written on sync. The first reading also plants flat week/month anchors so week_delta starts at 0 instead of null; later refreshes that change the count are real growth. Ads come from Firecrawl Ad Library search on Instagram/Facebook sync, plus a Page library link when `pageAdLibrary.id` is on the payload. Paid vs organic counts posts with a scrape/caption/analysis sponsored signal versus the rest, and shows how many library ads are active. The full Active ads catalogue lives at `/ad-library` (`AdsController`, Inertia `ad-library/Index`, sidebar Ads) - not `/ads` or `pages/ads/*`, which ad blockers often blank (`ERR_BLOCKED_BY_CLIENT`). Dashboard only shows a two-title overview under Your tracked accounts with a View all link. Public scrapes do not reveal organic boosts. Never print "not in this build" on product UI. The dashboard page heading is "Competitor Analytics". The dashboard mix heading is "Your tracked accounts" and shows a face pile of those snitches (`snitches` prop). Faces overlap until hover or focus, then each takes its own slot and shows the name. Keep `.snitch-face-pile` as `flex-wrap: nowrap` with horizontal scroll - wrapping while hovered drops `:hover` and the pile flickers. Hashtags, keywords, and CTA language stay readable wrapping chips with the count visible. Do not overlap text the way the face pile overlaps avatars. CTA language is grouped into short ask types. NanoGPT names each type from the analysed lines and the result is cached. Chips show the type and count. Choosing a chip lists the original lines, and each line links to the newest post that used that ask. Do not ellipsize the type. If the model is unavailable, each line keeps a short label from its first words and the full line stays in the list. Do not invent phrases that were not analysed. The cadence charts sit first under the stat rail, above Your tracked accounts, with no section heading. The dashboard rail is 9 cells when insights load: 3 columns below xl (filled rows), and one 9-column row at xl via `:has(> :nth-child(9))` - never `repeat(8)` which orphans Posts with an ask. Rail hints use `.snitch-dash-rail-hint` (nowrap + ellipsis). Inside cadence, the heat map sits beside follower history, then platform split, format mix, time of day, and weekly volume share one row. Do not stretch the short column so a hole opens between the heat map and follower history. Dashboard first paint keeps `stats` + `snitches` immediate and defers `activity`/`insights`/`recent_posts`/`top_winners` together in one Inertia group `panel` so charts and the contact sheet land in a single paint - do not split back into `board`/`feed`. Always render the full 9-cell dash rail (3 live stats + 6 pending placeholders) so the grid does not jump from 3 to 9 when insights arrive. Mix columns keep their labels while loading. Soft-fade deferred content (`snitch-dash-soft-in`); dash contact-reveal is opacity-only. Keep app-shell grain static. `StippleBar` animates via CSS `animation-delay` (never per-mark `setTimeout` / Vue `visibleCount` thrash). The account page (`/tracking/{id}`) uses the same packing: a six-cell rail (including paid vs organic), heat map beside follower history, then format mix, time of day, and weekly volume on one row, then hashtags, keywords, and a compact Active ads overview (running count, up to two titles, View all → `/ad-library`) with CTA language full width under them. Do not dump a tall Ad Library title list on Show. Recent posts default to 12 and offer Load more posts (`posts_limit` query, capped at 96) when more exist for that account.
