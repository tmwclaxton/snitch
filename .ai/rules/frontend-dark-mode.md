...
- Keep `--snitch-on-spot` charcoal for text on mustard fills (seg active, spot buttons, yellow chips, any hover that paints `--snitch-spot`). Never leave `--snitch-ink` or `--snitch-stock` (cream in dark) on a yellow face.
