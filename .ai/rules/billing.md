...
## Surfaces
- Authenticated `/billing` - balance, platform subscribe, credit packs, vendor usage, stacked spend chart of Apify/NanoGPT/Firecrawl/TikHub (`UsageBillingService::spendSeries`) with `grain=day|week|month` (default day). Chart bars are a dense lattice of mini vendor logos from `/images/vendors/` (not colour stipple dots); legend uses the same marks on paper plates. Short recent-charges preview (8 rows) links to the full list.
