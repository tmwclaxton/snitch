...
`ExploreBillingService`: `explore.search` on Explore `q` / `custom_tag` (and MCP `explore_posts` with `q`); idempotent per user + normalised query. `explore.view` when opening a completed reel whose `social_account_id` is not among the user's tracked snitches (`kind=competitor`); free for tracked snitches; idempotent `explore.view:{user_id}:{post_id}`. Insufficient credits → Billing (web) or MCP error.
