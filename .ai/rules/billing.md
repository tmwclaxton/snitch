...
## Credits
Agent MCP `create_account` starts at £0 with **no trial**. Claiming (WorkOS bind) or website signup grants `claim_bonus_pence` (£5) once (`idempotency_key` `claim_bonus:{user_id}`) and starts a generic no-card trial of `subscriptions.trial_days` (default 7) on `users.trial_ends_at`. Unclaimed MCP users never get `trial_ends_at`. `UsageBillingService::ensureClaimedEntitlements` heals legacy browser users that have `workos_id` but null `claimed_at` (sets claimed_at, credits starter once, refreshes trial when they never received claim_bonus). Call it from paywall checks and WorkOS `findUsing` for existing workos_id matches. Each paid platform subscription invoice grants `subscription_bonus_pence` (£30) once per invoice (`idempotency_key` `subscription_bonus:invoice:{invoice_id}`) via Stripe `invoice.paid`. Stripe Checkout must **not** add a subscription trial - the first paid invoice should still grant plan credits. Subscribing clears `users.trial_ends_at`.
