...
`OnboardingController::store` dispatches `GenerateInfluencerBriefJob` after the brand is saved. The job uses the default filters, writes `influencer_brief` on the brand, and charges NanoGPT (`influencer.brief`). Soft-skip when balance is below the billable floor or a brief already exists. Manual Generate on `/influencers` (POST `influencers.brief`) and MCP `generate_influencer_brief` also persist `influencer_brief`. Typing in the Brief field autosaves via PATCH `influencers.brief.update` (debounced ~500ms) with a quiet Saved indicator - no toast spam.

## Multi-seed discovery
Order: NanoGPT model seed + Firecrawl search/propose + vendor native platform search (Apify, or TikHub when `ApifyMonthlyCapGate` is exhausted) -> merge/dedupe -> platform resolve (require `external_id`) with follower extraction. Do not invent Keep-ready creators from LLM memory alone; model seeds always go through resolve verify. Soft-complete when at least one profile verifies but fill is under `min_suggestions` (`completed` + `partial: true`). Hard-fail via `InsufficientInfluencerSuggestionsException` only when **zero** profiles verify. Probe locally with `php artisan snitch:probe-influencer-find --sync`.
...
Dedupe by `platform:handle`. Prefer candidates that already have follower counts (usually Apify/TikHub search). Interleave seed sources (`apify-search`, `tikhub-search`, `firecrawl`, `model-seed`) so one source cannot starve the verify queue. Instagram resolve uses `resultsType: details` so `followersCount` is available for band filtering. When a min/max follower band is set, **unknown (`null`) follower counts are rejected** - only known in-band counts enter the shortlist. Soft-complete thin runs (>=1 verified but under `min_suggestions`) as `completed` with `partial: true` instead of hard-failing; hard-fail only when zero verified. Seed-time drops known out-of-band Apify/TikHub rows before resolve.
...
## Keep / discard lock
While undecided suggestions remain for a completed run, block a new search - unless the run is `partial: true` (thin shortlist) or `failed`. Failed runs and partial completed runs may start a new search without finishing Keep/Discard. Keep persists the profile and starts sync; Discard records the decision. Both update the poll cache so the UI can clear the card. MCP agents may call `dismiss_influencer_suggestions` to clear a shortlist/report run without Keep. UI may hint: "Last search found fewer than 6 - you can keep reviewing or search again."
...
Suggestion rows include `followers` from verify / platform search when a band is requested (required for acceptance). Keep copies that onto `tracked_accounts.followers` (nullable only for legacy rows). UI shows formatted follower counts on Review queue and Kept cards (`unknown` when null). MCP `keep_influencer` / `list_influencers` include `followers`. Sync may refresh the value when a profile resolve payload includes `followers`.
