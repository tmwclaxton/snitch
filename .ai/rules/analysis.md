...
`ExploreMixService` orders the catalogue (and soft-ranks semantic hits after exact pins) with a seeded weighted shuffle among posts above `snitch.explore.min_quality_ratio` of the pool max (winner score, else engagement proxy). Weak posts trail so page one stays useful while the strong set rotates every `seed_bucket_hours`. Do not sort Explore by winner score alone or by unfiltered random. Tunables: `SNITCH_EXPLORE_*` / `config/snitch.php` `explore.*`.
