...
- The post page (`feed/Show`) is the exception: pass `interactive` so one official player loads in `.snitch-post-player` (wide enough to use, not the 3/4 polaroid crop). Instagram uses `/embed/` there so the carousel can be swiped. Keep the "Open on platform" control off that player. From the `lg` breakpoint the page is `.snitch-post-fit`: a compact two-column layout with the player on the left (4/5, or 9/16 for TikTok, YouTube, and Facebook), the caption under that player and its metrics, and the analysis notes on the right. Cap the player height with `svh`, but do not lock the page to the viewport or put scrollbars on the caption or notes. The page itself scrolls when the write-up is long. Do not put a fixed 52rem frame back on that player.
...
- Dashboard Latest posts, Feed, and Explore use `.snitch-contact-sheet-proof-fill` (`auto-fit` + `1fr`, min 12rem) so leftover canvas is not a right gutter. Keep 3/4 frames. Do not add `grid-cols-*` on those boards. Latest posts loads `frames` posts (default 6, clamped 1-24): two full rows of however many columns the sheet has, and the dashboard replaces that query as the width changes. The dashboard sheet also uses `.snitch-contact-sheet-dash` and `compact` cells (no second hook line) so that column lines up with Top winners. Account recent posts keep `.snitch-contact-sheet-rows` and also pass `compact`.
