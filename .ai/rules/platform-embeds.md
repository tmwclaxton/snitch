...
- Dashboard Latest frames, Feed, and Explore use `.snitch-contact-sheet-proof-fill` (`auto-fit` + `1fr`, min 12rem) so leftover canvas is not a right gutter. Keep 3/4 frames. Do not add `grid-cols-*` on those boards. Dashboard Latest frames also sets `.snitch-contact-sheet-rows` with `--snitch-sheet-cols: 3` so the six posts fill two rows instead of a short second row with an empty gutter. The rows rule wins over proof-fill.
