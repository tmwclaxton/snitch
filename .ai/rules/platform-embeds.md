...
- The post page (`feed/Show`) is the exception: pass `interactive` so one official player loads in `.snitch-post-player` (wide enough to use, not the 3/4 polaroid crop). Instagram uses `/embed/` there so the carousel can be swiped. Keep the "Open on platform" control off that player. From the `lg` breakpoint the page is `.snitch-post-fit`: height `calc(100svh - 4rem)` under the app header, the player fills the left column (4/5, or 9/16 for TikTok, YouTube, and Facebook), and the notes column scrolls. Do not put a fixed 52rem frame back on that player.
