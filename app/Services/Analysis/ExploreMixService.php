...
class ExploreMixService
{
    ...
    /**
     * Stable seed so pagination stays consistent within a bucket, then rotates.
     */
    ...
    public function mix(array $scoredIds, int $seed): array
    {
        ...
        $mixedStrong = $this->weightedShuffle($strong, $seed);
    ...
    public function mixSemantic(array $scoredIds, array $pinnedIds, int $seed): array
    {
        ...
        $pinnedOrder = $this->weightedShuffle($pinned, $seed ^ 0x9E3779B9);
    ...
    private function weightedShuffle(array $scoredIds, int $seed): array
    {
        ...
        $jitter = max(0.0, min(1.0, (float) config('snitch.explore.jitter', 0.55)));
